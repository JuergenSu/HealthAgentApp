...
use App\OpenAI\OpenAiResponseMapper;use PHPUnit\Framework\TestCase;
...
final class OpenAiResponseMapperTest extends TestCase {
 ...
 public function testRejectsUnknownTool():void{$this->expectException(\InvalidArgumentException::class);(new OpenAiResponseMapper())->map(['output'=>[['type'=>'function_call','call_id'=>'x','name'=>'read_entire_room_database','arguments'=>'{}']]],null);}
 public function testRejectsMalformedFinal():void{$this->expectException(\InvalidArgumentException::class);(new OpenAiResponseMapper())->map(['output'=>[]],'{"type":"FINAL"}');}
}
